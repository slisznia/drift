...
Each case lives under `lang2/tests/codegen/e2e/<case>/` and must provide:
...
The runner:
  ...
  5) Asserts diagnostics are empty and compares exit/stdout/stderr to expected.json.
	...
	Artifacts are written to `build/tests/lang2/tests/codegen/e2e/<case>/`.
	"""
...
def main(argv: Iterable[str] | None = None) -> int:
	...
	ap.add_argument(
		"cases",
		nargs="*",
		help="Specific test case names to run (directories under lang2/tests/codegen/e2e)",
	)
	ap.add_argument(
		"-j",
		"--jobs",
	...
	case_root = ROOT / "lang2" / "tests" / "codegen" / "e2e"
	case_dirs = (
		sorted(
			d
