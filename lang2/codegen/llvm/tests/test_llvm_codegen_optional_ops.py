...
def test_optional_ops_round_trip_payload():
	...
	sig_dv_int = FnSignature(name="drift_dv_int", param_type_ids=[int_ty], return_type_id=dv_ty, declared_can_throw=False)
	fn_dv_int = FnInfo(name="drift_dv_int", declared_can_throw=False, return_type_id=dv_ty, signature=sig_dv_int)
	sig_err_new = FnSignature(
		name="drift_error_new_with_payload",
		param_type_ids=[int_ty, dv_ty],
	...
	entry = BasicBlock(
		...
		instructions=[
			...
			Call(dest="dv", fn="drift_dv_int", args=["payload_int"]),
			Call(dest="err", fn="drift_error_new_with_payload", args=["code", "dv"]),
