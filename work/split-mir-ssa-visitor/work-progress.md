...
- Throw summaries are aggregated in stage3 (`ThrowSummaryBuilder`) and consumed in stage4 (`throw_checks`) to build function-level `FuncThrowInfo` and enforce can-throw invariants: (1) a function not declared can-throw must not construct an Error; (2) a can-throw function must not contain a bare `return` with no value; (3) thrown events must be a subset of declared events when provided; (4) non-can-throw functions must not return a value typed as FnResult on typed paths; (5) FnResult shape is enforced structurally for untyped/unit tests and type-aware when SSA + `TypeEnv` are provided (typed paths skip the structural alias/forward guard and additionally compare the returned FnResult parts against the checker-declared `return_type_id` when available). The shared `TypeEnv` protocol lives in `lang2/core/types_protocol.py` (type-centric with `is_fnresult`/`fnresult_parts`). Testing impls live in `lang2/core/types_env_impl.py`: `SimpleTypeEnv` for manual tagging and `InferredTypeEnv`/`build_type_env_from_ssa` to derive FnResult types from SSA + signatures; checker-owned TypeCore (`lang2/core/types_core.py`) + TypeEnv bridge (`lang2/checker/type_env_bridge.py`) wrap inferred types into checker TypeIds, and a minimal checker TypeEnv builder tags return SSA values with signature TypeIds. Tests live in `lang2/stage4/tests/test_throw_checks.py`, `test_throw_checks_typeaware.py`, `test_throw_checks_typeaware_forwarding.py`, `lang2/stage4/tests/test_declared_events_invariant.py`, `lang2/stage4/tests/test_non_can_throw_returns_fnresult.py`, `lang2/stage4/tests/test_scalar_return_mismatch_typeaware.py`, `lang2/types_env_impl_tests/test_inferred_type_env.py`, `lang2/checker/tests/test_checker_type_env.py`, `lang2/tests/test_driftc_non_can_throw_returns_fnresult_integration.py`, and the try-sugar positive integration in `lang2/tests/test_driftc_try_sugar_ok_integration.py`.
- Codegen: `lang2/codegen/llvm/` now exposes an SSA-first textual LLVM emitter seeded with the v1 ABI types (`%DriftError`, `%FnResult_Int_Error`). The primary API is `lower_ssa_func_to_llvm(func, ssa, fn_info, fn_infos)`; MIR-based lowering was removed. SSA now carries `block_order` (deterministic RPO) and `cfg_kind` to make CFG support explicit. Control flow is still limited to straight-line/diamonds (loops rejected), and codegen now hard-fails on any cfg_kind outside `{STRAIGHT_LINE, ACYCLIC}`. Smoke tests live in `lang2/codegen/tests/test_llvm_codegen_basic.py`, `lang2/codegen/tests/test_llvm_codegen_controlflow.py`, module/call ABI tests (`lang2/codegen/tests/test_llvm_codegen_module_calls.py`), negative guards (`lang2/codegen/tests/test_llvm_codegen_negative.py`), and clang-driven e2e smoke tests (`lang2/codegen/tests/test_llvm_codegen_e2e.py`). Tests run via `just lang2-codegen-test` (included in `lang2-test`), which now cleans `build/tests/lang2` between runs. A clang-based IR runner (`lang2/codegen/ir_cases/e2e_runner.py`) executes prebuilt IR cases under `lang2/codegen/ir_cases/<case>/`, and a Drift-source runner (`lang2/codegen/e2e/runner.py`) now drives a simple source case (`simple_return`) through AST→HIR→MIR→SSA→LLVM→clang with artifacts in `build/tests/lang2/`. A top-surface driftc helper `compile_to_llvm_ir_for_tests` now uses the same SSA built for throw checks and is exercised by `lang2/driver/tests/test_driftc_codegen_e2e.py` (scalar main exits 42, can-throw callee returning FnResult.Ok exits 1).
- Parser copy: lang/ parser + grammar are mirrored under `lang2/parser/` with a thin adapter (`parse_drift_to_hir`) that maps parser AST to lang2.stage0 AST and signatures, then uses `AstToHIR`. Minimal parser tests live in `lang2/parser/tests` (including HResultOk sugar and attribute-call non-rewrite); `lang2-parser-test` is part of `lang2-test`. Drift-source e2e runner uses this path (no regex stubs). ThrowStmt adapter is wired; parser-emitted RaiseStmt for `throw` is temporarily mapped to ThrowStmt until real rethrow semantics exist.
