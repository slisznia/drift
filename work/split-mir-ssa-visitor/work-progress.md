...
- **Driver/checker integration:** Wire `declared_can_throw` from the type checker (FnResult returns / throws clauses) into stage4; driver should call `run_throw_checks(funcs, summaries, declared_can_throw)` after stage3, emitting diagnostics instead of RuntimeError. A stub driver (`lang2/driftc.py`) and richer checker scaffold (`lang2/checker/__init__.py`) now mirror the future shape (`FnInfo`, `CheckedProgram`, exception catalog placeholder, TypeEnv slot) and already thread `declared_can_throw` through the checker.
- **Type-aware FnResult returns:** Typed paths already run `enforce_fnresult_returns_typeaware` when SSA + TypeEnv are provided; structural alias/forwarding guard is only enforced on untyped/unit-test paths. Next step: relax/remove the structural guard for typed pipelines once a real TypeEnv is available everywhere.
- **Catch-arm validation in checker:** Lowering now rejects multiple catch-alls and catch-all-not-last; checker should additionally enforce duplicate/unknown event arms with user-facing errors (helpers in `lang2/checker/catch_arms.py`).
