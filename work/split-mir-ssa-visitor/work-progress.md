...
- AST→HIR visitor under `lang2/stage1/ast_to_hir.py` now lowers literals, vars, unary/binary ops, field/index, let/assign/if/while/for/return/break/continue/expr-stmt, and plain/method calls; for-loops desugar to `iter()/next()/is_some()/unwrap()` over Optional using HLoop/HIf/HBreak and scoped temps. Remaining sugar (ternary, try/throw/raise, array literals) still stubbed. Basic unit tests live in `lang2/stage1/tests/test_ast_to_hir.py`.  
...
- MIR→SSA now rewrites single-block straight-line MIR into SSA by replacing local load/store with `AssignSSA` moves, recording SSA versions per local and per-instruction (`value_for_instr`); multi-block functions are still rejected in the main entry point. An experimental multi-block path (`run_experimental_multi_block`) places Φ nodes for locals defined along multiple branches in a simple diamond CFG; covered by `lang2/stage4/tests/test_mir_to_ssa_multi_block.py`. Unit tests for single-block SSA live in `lang2/stage4/tests/test_mir_to_ssa.py`.  
