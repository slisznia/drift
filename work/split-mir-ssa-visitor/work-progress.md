...
- Stage2→3→4 integration is exercised in `lang2/stage4/tests/test_throw_checks_integration.py`, covering can-throw vs non-can-throw functions, try/catch paths with explicit `FnResult.Ok` returns, and the current structural rejection of FnResult forwarding/aliasing (expected to relax once type-aware checks land). A placeholder `TypeEnv` protocol and TODO hook for a type-aware check live in stage4/throw_checks.py. Additional integration cases pin the forwarding/aliasing limitation as expected-fail until the type-aware check replaces it.
...
- **More integration coverage:** Add more stage2→3→4 integration cases (e.g., try/catch with Ok return is covered; consider an expected-fail for FnResult forwarding until type-aware checks land).
