...
- HIR→MIR builder/skeleton under `lang2/stage2/hir_to_mir.py` lowers straight-line HIR (literals/vars/unary/binary/field/index + let/assign/expr/return), `if` with branches/join, `loop`/break/continue, basic calls/DV construction, ternary expressions (diamond CFG storing into a hidden temp), and `throw` (ConstructError + ResultErr + Return). `try` is partially supported: a try stack routes `throw` to the nearest catch block via locals/CFG; more advanced try/rethrow/array literals remain TODO. Unit tests in `lang2/stage2/tests/test_hir_to_mir*.py` cover these paths.  
