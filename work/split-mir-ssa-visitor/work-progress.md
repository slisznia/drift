...
- Throw summaries are aggregated in stage3 (`ThrowSummaryBuilder`) and consumed in stage4 (`throw_checks`) to build function-level `FuncThrowInfo` and enforce the first can-throw invariant: a function not declared can-throw must not construct an Error. Tests live in `lang2/stage4/tests/test_throw_checks.py`.  
