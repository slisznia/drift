...
- Stage2→3→4 integration is exercised in `lang2/stage4/tests/test_throw_checks_integration.py`, covering can-throw vs non-can-throw functions and try/catch paths with explicit `FnResult.Ok` returns. Additional structural limits (FnResult forwarding/aliasing) are pinned via throw-check tests until type-aware checks land.  
...
- **Result-driven try sugar:** Integrate the desugaring pattern (`val tmp = fallible(); if tmp.is_err() { throw tmp.unwrap_err(); } val x = tmp.unwrap();`) into the pipeline (likely a checker/HIR rewrite using the new `HTryResult`), including type checks that the operand is `FnResult<_, Error>`.
