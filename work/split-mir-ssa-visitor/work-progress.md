...
- Throw summaries are aggregated in stage3 (`ThrowSummaryBuilder`) and consumed in stage4 (`throw_checks`) to build function-level `FuncThrowInfo` and enforce can-throw invariants: (1) a function not declared can-throw must not construct an Error; (2) a can-throw function must not contain a bare `return` with no value; (3) a can-throw function must return a value produced by `ConstructResultOk/Err` (FnResult shape). A shared `TypeEnv` protocol now lives in `lang2/types_protocol.py` (now type-centric with `is_fnresult`/`fnresult_parts`) and is imported by stage4 for the future type-aware check. Tests live in `lang2/stage4/tests/test_throw_checks.py`.  
- Stage2→3→4 integration is exercised in `lang2/stage4/tests/test_throw_checks_integration.py`, covering can-throw vs non-can-throw functions, try/catch paths with explicit `FnResult.Ok` returns, result-driven try sugar (via `normalize_hir`) flowing through lowering and throw checks, and the current structural rejection of FnResult forwarding/aliasing (expected to relax once type-aware checks land). A placeholder `TypeEnv` protocol and TODO hook for a type-aware check live in stage4/throw_checks.py; integration cases pin the forwarding/aliasing limitation as expected-fail until the type-aware check replaces it. A stub driver (`lang2/driftc.py`) and checker placeholder (`lang2/checker/__init__.py`) illustrate where `declared_can_throw` will come from in the real compiler.
...
- **Driver/checker integration:** Wire `declared_can_throw` from the type checker (FnResult returns / throws clauses) into stage4; driver should call `run_throw_checks(funcs, summaries, declared_can_throw)` after stage3, emitting diagnostics instead of RuntimeError. A stub driver (`lang2/driftc.py`) and checker scaffold (`lang2/checker/__init__.py`) live in the repo to hang this integration on once the real checker exists.
