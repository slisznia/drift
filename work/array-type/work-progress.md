...
- Pipeline plumbing: shared `TypeTable` passed from resolver into Checker; guard added so TypeId-carrying signatures must bring the matching table. The checker now reuses pre-seeded scalar/error types on shared tables (no duplicate TypeIds) and derives missing error_type_id from FnResult return types when present. SSA typing reuses the shared String `TypeId` to keep HIR/SSA consistent. Drift-source e2e runner passes cleanly for `simple_return` (no throw-check mismatches).
